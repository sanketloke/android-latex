 ...
 * 
 */
public class RenameFile extends LoadManagement implements IURLLoaded {
	private int _con;
	private String _login;
	private String _pwd;
	private IRenameFile _parent;
	private Context _context;
	private String _project;
	private String _name;
	private String _new_name;

	public RenameFile(Context context, IRenameFile parent, String login,
			String pwd, String project, String name, String new_name) {
		super(context);
		_context = context;
		_login = login;
		_pwd = pwd;
	...
	public void compute() {
		...
		LoadURL _url = new LoadURL(this, 1, getUrl()
				+ ActionURLS.getUpdateFileNameURL(_login, _pwd, 
						_project, _name, _new_name), null);
	...
	public void loadEnd(int serial, String text, ProgressDialog progress) {
		...
		if (_error) {
			...
			_parent.onRenameFileFailure("log", null);
			return;
		}
		JSONObject _objet;
		try {
			...
			if (_objet.has("updated") && "ok".equals(_objet.optString("updated"))
				...
				_parent.onRenameFileSuccess(res,_name, _new_name,
						_objet.optString("version"));
			} else {
				_parent.onRenameFileFailure(_context.getString(R.string.filerenameerror),
						_objet.optString("version"));
			}
		} catch (JSONException e) {
			_parent.onRenameFileFailure("segfault " + text,null);
			e.printStackTrace();
		}
